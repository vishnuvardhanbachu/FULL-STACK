...
// Get employer info
$eq = $pdo->prepare("SELECT company_name FROM users WHERE id=?"); $eq->execute([$other_user]); $employer=$eq->fetch();
$jq = $pdo->prepare("SELECT title FROM jobs WHERE id=?"); $jq->execute([$job_id]); $job=$jq->fetch();
if(!$employer || !$job) die("Not found");
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Chat with <?=e($employer['company_name'])?> — JobPortal</title>
...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a class="nav-item active" href="dashboard.php#applications"><span class="icon">📋</span> My Applications</a>
...
<main class="main-content">
  <div class="topbar">
    ...
    <div class="topbar-right">
      <a href="dashboard.php#applications" class="btn btn-ghost btn-sm">⬅ Back to Dashboard</a>
    </div>
  </div>
  ...
  <div class="page-content">
    <div class="chat-box animate-in">
      <div class="chat-header">
        <div>
          <div style="font-size:1.1rem;"><?=e($employer['company_name'])?></div>
